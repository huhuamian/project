...
/*******************************Top动画***************************************************/																		
	...
	if(fn(location.search,"username")){
	$(".load dt a span").html(fn(location.search,"username"))
	$(".loge dt a").html("退出").attr("href","index.html")
	}
	...
	$.get("../json/storesame.json", function(arr) {
		for( var i=0;i<arr.length;i++){
			...
			for(var j=0;j<Top.length;j++){
				if(j==0){
					var storesametop=$("<div class='"+Top[j].clas+"' style='margin-right:8px'><div class='storesametoptxt';><p class='enbrand'>"+Top[j].ename+"</p><p>"+Top[j].cname+"</p><p>"+Top[j].price+"</p></div><div class='storesametoppic';><a href='DetailPage.html?id="+Top[j].id+"'><img src='"+Top[j].src+"' width='150'></a></div></div>")
			li.find(".storesame_top").append(storesametop)
				}
				else if(Top[j].clas=="storesame_top2"){
					var storesametop=$("<div class='"+Top[j].clas+"'><div class='storesametoptxt';><p class='enbrand'>"+Top[j].ename+"</p><p>"+Top[j].cname+"</p><p>"+Top[j].price+"</p></div><div class='storesametoppic storesametoppic2';><a href='DetailPage.html?id="+Top[j].id+"'><img src='"+Top[j].src+"' width='150'></a></div></div>")
			li.find(".storesame_top").append(storesametop)
				}
				else{
					var storesametop=$("<div class='"+Top[j].clas+"'><div class='storesametoptxt';><p class='enbrand'>"+Top[j].ename+"</p><p>"+Top[j].cname+"</p><p>"+Top[j].price+"</p></div><div class='storesametoppic';><a href='DetailPage.html?id="+Top[j].id+"'><img src='"+Top[j].src+"' width='150'></a></div></div>")
			li.find(".storesame_top").append(storesametop)
				}
			}
			for(var l=0;l<Bottom.length;l++){
				var Botobj=Bottom[l]
			var storesamebottom1=$("<div class='"+Botobj.clas+"'><div class='storesametoptxt';><p class='enbrand'>"+Botobj.ename+"</p><p>"+Botobj.cname+"</p><p>"+Botobj.price+"</p></div><div class='storesametoppic';><a href='DetailPage.html?id="+Botobj.id+"'><img src='"+Botobj.src+"' width='150'></a></div></div>")
			li.find(".storesame_bottom").append(storesamebottom1)
			}
		}
		$(".storesame_top>div").click(function(){
...
function shopcart(){
			var arr = $.cookie("cart");
					if (arr) {
						...
						//先清空旧节点
						$(".shopping_cart_count").empty(); 
						
						//如果购物袋中有商品
						if (arr.length > 0) {
						$("<div class='v30hmc_head'>购物袋中的商品</div>").prependTo(".shopping_cart_count")						
							//再添加新节点
							//遍历arr
							console.log(22)
							var total = 0; //总价
							...
							var  li=$("<ul><li><div class='v30hmc_box'><div class='v30hmcb_i'><img src='"+obj.src+"'></div><ul class='v30hmcb_l'><li>"+obj.ename+"</li><li>"+obj.color+","+obj.Size+"</li><li><span>"+obj.price+"</span></li></ul><div class='clearboth'></div></div></li></li></ul>").appendTo(".shopping_cart_count")
							...
							$("#cart_number").html(numbb)
						$("<div class='v30hmc_foot'>共<span class='cod00000'>"+numbb+"</span>件商品<br>共计<span class='right_menu-qrcode-tip0000 w5000'>￥"+total+"<span><a href='cart.html'><span class='v30_btred v30hmc_gc'>去购物车结算</span></a></div><b style='display:none;'></b>").appendTo(".shopping_cart_count")
						}else{
							$("<ul><li><p>购物袋暂时没有商品<br />赶紧选择心爱的商品吧！</p></li></ul>").appendTo(".shopping_cart_count")
						}
					}else{
						$("<ul><li><p>购物袋暂时没有商品<br />赶紧选择心爱的商品吧！</p></li></ul>").appendTo(".shopping_cart_count")
					}
